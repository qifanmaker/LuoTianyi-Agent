...
def main():
    ...
    target_change_interval = 0.3  # 每秒改变一次目标点
    random_x, random_y = display[0]/2, display[1]/2
    ...
    def on_start_motion_callback(group: str, no: int):
        ...
        audioPath = os.path.join(resources.CURRENT_DIRECTORY, "audio.wav")
    ...
    radius_per_frame = math.pi * 10 / 1000 * 0.4  # 降低摇摆速度
    deg_max = 2  # 将最大角度从5度减小到2度
    progress = 0
    ...
    while True:
        for event in pygame.event.get():
            ...
            #         pos.smooth_move_to(pos.target_dx - 0.1, pos.target_dy)
            #     elif event.key == pygame.K_RIGHT:
            #         pos.smooth_move_to(pos.target_dx + 0.1, pos.target_dy)
            #     elif event.key == pygame.K_UP:
            #         pos.smooth_move_to(pos.target_dx, pos.target_dy + 0.1)
        ...
        if wavHandler.Update():
            # 利用 wav 响度更新 嘴部张合
            # print("Rms: ",wavHandler.GetRms() * lipSyncN)
